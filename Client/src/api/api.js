...
// Interceptor de resposta — redireciona ao login em 401
...
export const autenticacaoApi = {
  login:  (email, senha)  => cliente.post('/api/auth/login', { email, password: senha }),
  perfil: ()              => cliente.get('/api/auth/me'),
};
...
export const dashboardApi = {
  ...
  ocorrenciasPorSetor: ()       => cliente.get('/api/dashboard/occurrences-by-sector'),
  recentes:           (limite = 5) =>
...
export const relatoriosApi = {
  ...
  episAusentes:         (filtros) => cliente.get('/api/reports/missing-epis', { params: filtros }),
  riscoSetor:           (filtros) => cliente.get('/api/reports/sector-risk', { params: filtros }),
  exportarCsv:          (filtros) =>
    cliente.get('/api/reports/export-csv', { params: filtros, responseType: 'blob' }),
};
...
export const configuracoesApi = {
  ...
  salvarNotificacoes:  (dados) => cliente.put('/api/settings/notifications', dados),
};
